...
## 0.1 차별화 가치 선언 — 의도적으로 *하지 않는* 것

같은 시장의 다른 강의·플랫폼 분석 (`docs/dewey/`) 후 내린 결론. *흉내내지 않는 다섯 가지* 가 곧 우리 정체성.

1. **엘리트 프레이밍 채택 X.** *"실리콘밸리 상위 1%"*, *"엔터프라이즈급"* 같은 어휘는 빌더 입문자(핵심 페르소나) 를 도망가게 한다. 우리는 *수평적 입문* 톤이 정체성. *"누구나 시작점은 같다."*
2. **광범위 인구 호명 X.** *"N만 명을 위한 가이드"* 같은 호명은 강사 브랜드가 있어야 작동. 우리는 *수신자 한 명* 을 부른다 — *"AI를 처음 만지는 당신을 위해."*
3. **강사 인물 의존 X.** 권위는 *방법론(Harness Engineering) + 12 Phase 구조 + 7-step 루프* 에서 온다. 인물 사진·이력에 의존하지 않는다.
4. **사전지식 가정 X.** *"사전지식 필요"* 라벨을 의도적으로 회피한다. 첫 약속은 *"코딩 0, 프롬프트 0이라도 OK."*
5. **도구 종속 X.** Claude Code · Codex · Cursor · Gemini 가 다 바뀌어도 살아남는 *빌더 마인드셋* 에 비중을 둔다. 도구는 *대표 예시*로만 등장.
