...
#### 프롬프트 연습장 — 2.0 인터랙티브 기능 · ✅ 1차 완료
(기존 v0.9 마일스톤을 2.0으로 흡수.) 레슨·개념 안에서 직접 프롬프트를 쓰고 본인 API 키로 실제 호출한다. 키는 localStorage 저장, 클라이언트 직접 호출 — 정적 사이트 유지.
**✅ 1차(2026-06-07)** — Gemini BYO 키(`lib/gemini.ts`·`useApiKey`·`PromptPlayground`), `Lesson.playgroundPrompt`로 레슨에 삽입(개념 깊이 아코디언 다음 — "개념 → 즉시 실습"). 핵심 레슨 6곳(프롬프트 구조·출력 형식·장문 요약·환각 검증·보고서 초안·이슈 변환)에 적용. Concept 노드 연동은 다음.
...
- **줄기 A·B 진행** (2026-06-07) — neededWhen 12→33 · conceptDepth 3→12 · 산출물 포트폴리오(/me) · 운영자 분석(Plausible env 게이팅) · 프롬프트 연습장 1차(Gemini BYO). 전부 prod 반영.
- **다음** — 프롬프트 연습장 확대·Concept 연동, 운영자 분석 2차(막힘·구제 이벤트), 입력 파이프라인.
