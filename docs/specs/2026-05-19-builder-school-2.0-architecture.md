...
> 개정 — 2026-05-19 스펙 리뷰 1차 반영(레슨 수 정정, 노드 id 규칙, NodeMeta 배치, Concept·Special 인터페이스, 검증 규칙 구체화).
...
1.0은 13개 Phase 사다리 · 61개 레슨 · 6개 Journey의 **선형 커리큘럼**이다. 두 보고서가 한계를 드러냈다.

- 20인 베타 시뮬레이션 — 완주율 30%, NPS −35, L1 학습자 완주율 0%. 선형 사다리 하나로 입문자와 숙련자를 동시에 만족시키지 못한다.
- 자체 개선 보고서 — 자동 생성 레슨에 AI 문체 잔존, 명칭 불일치, 정합성 균열.

더 근본적인 문제는 **AI 시장의 변동성**이다. 특정 제품·버전에 묶인 콘텐츠는 빠르게 낡고, 그때마다 수십 개 레슨을 손봐야 한다. 1.0의 평평한 구조는 이 변동을 흡수할 장치가 없다.

### 0-2. 2.0의 한 줄 정의

AI Builder School 2.0은 **입력으로 자라는 살아있는 지식 그래프**다. 원리는 항구적 코어로 안정되고, 제품 종속 정보는 휘발성 엣지로 분리되어, 급변하는 AI 시장에서 항상성을 유지한다.

### 0-3. 설계 목표 (4번 항목 "규모 +50% · 시간 감소"의 해석)

- **시스템은 +50% 성장한다** — 그래프 레이어 · 입력 파이프라인 · 특강 체계가 새로 생긴다.
- **필수 강의 시간은 줄어든다** — 현재 61개 레슨을 통합·감축하고, 살아남는 레슨은 각각 2배 깊어진다. 그래프 기반 개인화 경로로 학습자별 필수 경로가 짧아진다. 통합 후 목표 레슨 수와 구체 매핑은 레슨 심화 스펙(2번)에서 확정한다.
...
이번 세션의 명확화 질문으로 확정된 분기다.

| 결정 | 선택 | 함의 |
|---|---|---|
| 온톨로지 깊이 | **그래프는 연결층** | Phase 사다리는 유지. 그 위에 그래프를 얹는다. 전면 노드 컬렉션(척추)은 기각 |
| 규모 방향 | **레슨 통합·감축, 시스템이 +50%** | 현재 61레슨을 통합·감축, 각 2배 심화 |
...
`Phase`와 `Journey`는 노드가 아니라 **렌즈(뷰)**다. 그래프를 선형으로 보여주는 방식이며 §4-3에서 정의한다.
...
- 렌즈(Phase·Journey)의 기본 표시에서는 제외된다 — "보관 자료" 토글로만 노출.

### 2-3. Concept 노드의 범위 제한

이번 세션은 Concept "노드 타입과 관계"를 정의하는 데까지만 한다. 개념 위키 항목을 실제로 집필하는 일은 별도 콘텐츠 작업이다. 마이그레이션에서는 기존 레슨의 `coreConcepts`를 Concept 노드 *초안*으로 자동 유도하며, 같은 개념이 여러 레슨에 등장하므로 `term` 기준 중복 제거 로직이 필요하다(§7 참조).
...
- 기존 콘텐츠 객체의 `id` 필드(`lesson-01`, `project-doc-qa` 등)는 **그대로 둔다** — 61개 객체를 건드리지 않는다. 그래프 노드 id는 각 노드의 `slug`에서 *유도*한다.
...
| `partOfJourney` | Lesson·Special → Journey | 어느 여정 경로에 속하는가 (Project는 `requiredPhases`로 간접 연결 — §9) |
...
- `src/content/phases.ts` · `src/content/journeys.ts` — **유지**. Phase·Journey의 풍부한 필드(`titleKo`·`level`·`estimatedHours`·`outcomes` 등)는 그대로 두고, 렌즈는 이 파일에서 *유도*된다(§4-3).
...
- `src/lib/content.ts` — 그래프 질의 헬퍼(역참조·관련 노드·렌즈 전개) 추가.
...
기존 `Lesson`·`Project`·`ContentTemplate` 인터페이스에는 `NodeMeta` 필드를 옵셔널로 합친다(`interface Lesson extends NodeMeta`). 옵셔널이므로 기존 61개 레슨 객체는 수정 없이 컴파일된다.

### 4-3. 렌즈 — Phase와 Journey

Phase와 Journey는 노드가 아니라 렌즈다. 단 **`phases.ts`·`journeys.ts` 파일은 삭제되지 않는다.**

- `phases.ts`의 `Phase` 객체는 `titleKo`·`level`·`estimatedHours`·`outcomes` 등 렌즈에 없는 풍부한 메타데이터를 계속 보유한다. `getPhaseById()` 같은 기존 호출 경로는 그대로 동작한다.
- `ontology.ts`의 렌즈는 `phases.ts`에서 *유도*된다 — `Phase.lessonSlugs` 배열이 `Lens.nodeIds`(순서 있는 `{kind}:{slug}` 목록)로 변환된다. 렌즈에는 레슨뿐 아니라 Special·Project도 끼울 수 있다(예 — Phase 4 끝에 "코덱스 신기능 특강").
- Journey 렌즈도 같은 방식 — `journeys.ts`의 `recommendedLessons` 등에서 유도된다.

즉 `phases.ts`·`journeys.ts`는 Phase·Journey의 정본으로 남고, 렌즈는 그것을 그래프 뷰로 투영한 파생물이다.
...
1번 항목 "성장 엔진"의 핵심이며, 아키텍처가 반드시 정의하는 **노드 수용 계약(intake contract)**이다. 파이프라인의 세부 메커니즘(정제 에이전트·intake 폴더 포맷·분류 휴리스틱)은 1번 하위 시스템의 별도 스펙이다. 이 문서는 흐름과 계약만 고정한다.
...
- Phase 렌즈가 새 Special을 제 위치에 자동 노출
...
1. **타입 확장 (무손상)** — `types.ts`에 `NodeMeta` 필드를 *옵셔널*로 추가하고 `Lesson`·`Project`·`ContentTemplate`에 합친다. 새 인터페이스(`Concept`·`Special`·`Edge`·`Lens`)를 추가한다. 옵셔널·추가이므로 기존 61레슨 무영향.
2. **`ontology.ts` 신설** — 기존 61레슨·6프로젝트·7템플릿을 노드로 등록(`{kind}:{slug}` id 유도). 엣지 상당수를 기존 데이터에서 자동 유도한다.
	...
	- `Lesson.coreConcepts` → `Concept` 노드 초안 + `teaches` 엣지 (`term` 기준 중복 제거)
3. **Phase·Journey를 렌즈로 투영** — `phases.ts`·`journeys.ts`는 유지하고, `ontology.ts`가 거기서 렌즈를 유도한다(§4-3).
4. **about 페이지 2.0 비전 섹션** 추가(§8).
5. 이후 — 레슨 통합·감축 · 특강 · 입력 파이프라인은 별도 하위 스펙.

### 7-1. 휘발성 기본값 부여

마이그레이션 시 기존 노드는 `volatility`가 없다. 일괄 기본값을 적용한다 — Lesson·Concept는 `evolving`, Project는 `evolving`, Template은 `evolving`. 이후 사람이 검토하며 `evergreen`/`volatile`로 조정한다. Special은 신규 생성 시 항상 `volatile`.
...
- **`targetJourneys`** — 정본은 `Lesson` 객체의 `targetJourneys` 필드다. `partOfJourney` 엣지는 거기서 유도되는 읽기 전용 파생물이며, 손으로 따로 관리하지 않는다. `Project`에는 `targetJourneys` 필드가 없으므로, Project의 여정 연결은 `Project.requiredPhases` → 해당 Phase의 `targetJourneys`로 간접 유도한다.
- **`prerequisites`** — 정본은 콘텐츠 객체의 `prerequisites` 필드. `prerequisite` 엣지는 파생물.
...
3. **2번 — 레슨 심화** — 별도 스펙. 61레슨 통합·감축, 각 2배 심화, AI 문체 교정.
...
- DSS와의 타입 공용 패키지 추출 여부는 별도 통합 스펙에서 정한다.
