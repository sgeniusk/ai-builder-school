...
> 상태 — 승인됨 · §0 구현 완료(`claude/2.0-foundation-v2`) · 작성 2026-05-19 · 범위 §0(아키텍처 토대)에 한정
...
§0 토대 구현이 끝났다. 이후 진행 순서는 로드맵 검토(2026-05-19)에서 아래로 확정됐다 — `docs/roadmap.md` §2 참조.

1. **§0 구현 — 완료.** 타입 · `ontology.ts` · 기존 콘텐츠 노드 이관 · about 페이지.
2. **1순위 — 특강(Special Presentation)** — 별도 스펙(스펙 3). 인터랙티브 슬라이드 형식, 첫 특강 제작, `archived` 페이지 배너. §0이 만든 `volatile`·`deepens`·`supersedes` 구조를 가장 먼저 검증한다.
3. **2순위 — 레슨 통합·심화** — 별도 스펙(스펙 2). 84레슨 통합·감축, 각 2배 심화, AI 문체 교정.
4. **3순위 — 입력 파이프라인** — 별도 스펙(스펙 1). 정제·분류 에이전트, intake 폴더 포맷, 분류 휴리스틱. 그래프·노드 타입이 성숙한 뒤에 깐다.
5. **프롬프트 연습장** — 별도 스펙. BYO 키 클라이언트 호출 인터랙티브 기능을 Concept 노드에 엮는다(기존 v0.9를 2.0으로 흡수).

> 스펙 번호(1번 입력 파이프라인 · 2번 레슨 심화 · 3번 특강)는 상위 기획의 식별자라 그대로 두고, 위 1~5는 *실행 순서*다.
