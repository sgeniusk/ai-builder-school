...
## P2 이후 진행 로그
- ✅ **neededWhen 확대 (12→33)** 2026-06-07 — 막힘(blocker)이 보내는 rescueLesson 중 미보유 21개 전수 백필. 모든 막힘이 JIT 헤더 있는 레슨으로 착지(top-down "왜 지금" 100%). `npm run check` 그린. (줄기 A 콘텐츠 깊이 1차)
- ✅ **conceptDepth 확대 (3→12)** 2026-06-07 — `ontology.ts` teaches 엣지 기준, concepts.ts 10개 개념을 가르치는 레슨 9개에 4단 사다리(비유·실무·구현·원리) 백필. 10개 개념 100% 커버. `npm run check` 그린. (줄기 A 콘텐츠 깊이 2차)
- ✅ **산출물 포트폴리오** 2026-06-07 — 프로젝트 상세에 완성 기록(체크+결과물 링크+회고, `usePortfolio` localStorage `aibs:portfolio:v1`) + /me "내가 지은 것" 섹션. **발견** — `done`이 이미 build·verify·reflect 100% 집계라 "완강률"이 사실상 검증·회고 반영. 진짜 gap은 산출물 전시였음. `npm run check` 그린. (줄기 A 평가 모델)
- ✅ **운영자 학습 분석 1차** 2026-06-07 — Plausible 경량(`lib/analytics.ts` + layout 스크립트). `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` env 게이팅 — 미설정이면 추적 0(무백엔드 유지), 운영자가 켤 때만 활성. 페이지뷰 + `Project Built` 커스텀 이벤트, 쿠키리스·익명 + footer 고지. **운영자 작업 대기** — Plausible 계정·도메인 등록 + `.env.local`에 env 설정. 막힘·구제 클릭 이벤트는 2차(클라이언트 래퍼 필요).

## 남김(P2 이후)
- ~~진척 모델 재정의 — 산출물·검증·회고 1급화~~ — 사실상 완료. `done`이 이미 build·verify·reflect 100% 집계라 검증·회고가 1급 반영돼 있었음(2026-06-07 발견). 산출물은 포트폴리오로 보강.
- ~~산출물 포트폴리오 — /me 개편~~ — ✅ 완료(위 진행 로그).
- ~~실시간 AI 튜터 (백엔드·API 키)~~ — **제외 결정**(2026-06-07): 컴패니언 되묻기 브리지가 무백엔드로 역할 대부분 수행, 한계효용 낮음.
- conceptDepth 나머지 백필 — 개념 10개 외 핵심 레슨 (선택·점진).
- ~~운영자용 학습 분석~~ — ✅ 1차 완료(Plausible env 게이팅, 위 진행 로그). 운영자 도메인 설정 시 활성. 막힘·구제 클릭 커스텀 이벤트는 2차.
