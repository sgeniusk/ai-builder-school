 ...
 *
 * 사용법:
 *   npm run new-lesson <slug> --stage <stage-N-slug> [--sub 4a] [--title "..."]
 *
 * 예시:
 ...
 *
 * v0.4 Stage 모델 — `--stage`는 필수. `--sub`은 Stage 4·6에서만 쓴다.
 *
 * 동작:
...
for (let i = 1; i < args.length; i += 2) {
  ...
  if (flag === "--title") titleKoArg = val;
  else if (flag === "--stage") stageSlugArg = val;
  else if (flag === "--sub") subGroupArg = val;
  else fail(`알 수 없는 플래그: ${flag}`);
}

// v0.4 Stage 모델 — `--stage`는 필수.
if (!stageSlugArg) {
  fail("--stage <stage-N-slug> 는 필수입니다. 예: --stage stage-2-ask");
}
const stageMatch = stageSlugArg.match(/^(stage-\d+)/);
if (!stageMatch) {
  fail(`--stage 값이 stage-N-* 형식이 아닙니다. 받은 값: "${stageSlugArg}"`);
}
const stageId = stageMatch[1] as string;
let stageOrdinal = 1;
...
// ── 4. lesson-stage-mapping.ts 읽고 stage ordinal · id 계산 (v0.4) ──
if (!fs.existsSync(lessonStageMappingPath)) {
  fail(`lesson-stage-mapping.ts를 찾을 수 없음: ${lessonStageMappingPath}`, 2);
}
const mappingRaw = fs.readFileSync(lessonStageMappingPath, "utf-8");
if (mappingRaw.includes(`"${slug}":`)) {
  fail(`이미 매핑된 slug: "${slug}". lesson-stage-mapping.ts에 존재.`);
}
// 해당 stage의 ordinal 최댓값 + 1
...
// lesson id — stage 기반: lesson-{S}{XX}
const stageNum = stageId.split("-")[1];
const newId = `lesson-${stageNum}${String(stageOrdinal).padStart(2, "0")}`;
if (lessonsRaw.includes(`id: "${newId}"`)) {
  fail(`id 충돌: "${newId}" 가 이미 lessons.ts에 존재합니다.`);
}
...
const newMappingRaw = mappingRaw.replace(
  mappingClosePattern,
  "\n" + mappingLine + "};\n\n/**",
);
...
console.log("  5. EXPECTED_STAGE_DISTRIBUTION·EXPECTED_TOTAL_LESSONS를 갱신합니다.");
